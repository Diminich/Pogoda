...
export const WeatherDataContainer: React.FC<WeatherDataContainerProps> = ({
  renderTodayWeather,
  renderTomorrowWether,
  renderDailyWether,
}) => {
  const currentWeatherData = useSelector<
    AppStateType,
    CityCurrentWeatherData[]
  >((state) => state.bodySearchCityPage.cityCurrentWeatherData);
  const hourlyWetherData = useSelector<AppStateType, CityHourlyWeatherData[]>(
    (state) => state.bodySearchCityPage.cityHourlyWeatherData
  );
  const dailyWeatherData = useSelector<AppStateType, CityDailyWeatherData[]>(
    (state) => state.bodySearchCityPage.cityDailyWeatherData
  );
